...
const statistics = {
    ...
    maxSongs: Math.max(...playlists.map(p => p.songs.length)),
    largestPlaylists: playlists.filter(p => p.songs.length === Math.max(...playlists.map(p => p.songs.length))).map(p => p.title)
};
